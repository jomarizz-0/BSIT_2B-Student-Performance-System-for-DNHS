...
require_once 'db.php'; // having the control to the database
...
switch ($method) {
    ...
    case 'POST':
        ...
        if (!preg_match('/^\d{8}$/', $lrn)) {
            http_response_code(422);
            echo json_encode(['errors' => ['LRN must be 8 digits only.']]);
            break;
        }
    ...
    case 'DELETE':
        ...
        $pdo->prepare("DELETE gd FROM grade_details gd JOIN grades g ON gd.grade_id = g.grade_id WHERE g.stud_lrn = ?")->execute([$id]);
        $pdo->prepare("DELETE FROM attendance WHERE stud_lrn = ?")->execute([$id]);
        $pdo->prepare("DELETE FROM grades WHERE stud_lrn = ?")->execute([$id]);
